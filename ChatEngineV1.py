    ...
    if event.message.text == '發發選單' or event.message.text == 'services':
        buttons_template = TemplateSendMessage(
            ...
            template=ButtonsTemplate(
                title='服務類型',
                text='請選擇',
                thumbnail_image_url='https://i.imgur.com/VplnrlW.jpg',
...
def retrieveTWAirCondition(regionname=None):
    rs=requests.session()
    res=rs.get(websites['AQI'].format(regionname))
    aqi_data = json.loads(res.text)
    print(aqi_data)
